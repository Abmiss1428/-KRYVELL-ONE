const APPS={
  ...
  nyxcore:{name:'NYXCORE',badge:'NYX',desc:'Écosystème de vie artificielle local : ADN, énergie, reproduction, pooling, PocketPal et lignées.',mode:'nyxcore',url:'/nyxcore-safe.html?v=0.8.10'},
...
function openApp(appKey){
  ...
  if(app.mode==='agentic'){
    windowChrome(appKey,'<div id="agenticCoreMount" style="height:100%;overflow:auto">Chargement de KRYVELL AGENTIC…</div>');
    const mount=document.getElementById('agenticCoreMount');
    if(window.KryvellAgentic?.render)window.KryvellAgentic.render(mount);else mount.textContent='KRYVELL Agentic indisponible.';
    return;
  }
  if(app.mode==='fusion'){
  ...
  if(app.mode==='nyxcore'){
    windowChrome(appKey,'<div id="nyxcoreMount" style="height:100%">🧬 Ouverture rapide de NYXCORE…</div>');
    const mount=document.getElementById('nyxcoreMount');
    if(window.KryvellNyxcore?.render){
      window.KryvellNyxcore.render(mount);
    }else{
      mount.innerHTML='<div class="window-placeholder"><h2>NYXCORE</h2><p>Le moteur local ne répond pas encore.</p><div class="window-links"><a href="/nyxcore-safe.html?v=0.8.10">OUVRIR ACCÈS DIRECT</a></div></div>';
    }
    return;
  }
  if(app.mode==='health'){
...
// Deep links from external launchers.
