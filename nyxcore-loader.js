/* NYXCORE SMART LOADER v1.0.0 — KRYVELL OS 0.7.1
...
(() => {
  ...
  function loadScript(src){
    return new Promise((resolve,reject)=>{
      ...
      if(existing){ if(existing.dataset.loaded==='1'||existing.readyState==='complete') return resolve(); existing.addEventListener('load',resolve,{once:true}); existing.addEventListener('error',()=>reject(new Error(`load_failed:${src}`)),{once:true}); return; }
  ...
  function seed(w,h){
    ...
    dots=Array.from({length:count},(_,i)=>({
      ...
      r:.7+Math.random()*1.6,hue:(i*47+Math.random()*55)%360,phase:Math.random()*Math.PI*2
    }));
  }
  ...
  function drawAmbient(t){
    ...
    const grid=56;for(let x=0;x<w;x+=grid){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke();}for(let y=0;y<h;y+=grid){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke();}
    for(const d of dots){
  ...
  function startAmbient(){
    if(ambientRunning||window.KryvellNyxcore) return;
  ...
  async function ensureFull(){
    if(window.KryvellNyxcore) return window.KryvellNyxcore;
    if(loading) return loading;
    stopAmbient();
    loading=(async()=>{
      for(const src of FULL) await loadScript(src);
      const started=Date.now();
      while(!window.KryvellNyxcore&&Date.now()-started<5000) await new Promise(r=>setTimeout(r,40));
      if(!window.KryvellNyxcore) throw new Error('nyxcore_not_ready');
  ...
  function prefetchWhenIdle(){
    if(!navigator.onLine||window.KryvellNyxcore) return;
    const run=()=>{
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden)stopAmbient();else if(!window.KryvellNyxcore)startAmbient();});
