...
export default async function handler(req,res){
  ...
  if(req.method==='GET'){
    return res.status(200).json({ok:true,configured:Boolean(GEMINI_API_KEY),provider:'google-gemini',model:DEFAULT_MODEL});
  }
  ...
  try{
    ...
    if(!response.ok){
      return res.status(502).json({ok:false,error:'gemini_request_failed',status:response.status,model});
    }
    const text=(data?.candidates?.[0]?.content?.parts||[]).map(p=>p?.text||'').join('').trim();
    if(!text) return res.status(502).json({ok:false,error:'gemini_empty_response',model});
    return res.status(200).json({ok:true,provider:'google-gemini',model,text});
  }catch(err){
    const error=err?.name==='AbortError'?'timeout':'cloud_unreachable';
    return res.status(504).json({ok:false,error,model});
  }finally{
    clearTimeout(timer);
  }
}
